<?php
    ...
    class ManageDB extends Conexion {
        ...
        public function __construct() {
            // el metodo get es heredado de Conexion
        ...
        /**
         * Devuelve una unica fila de una tabla.
         */
        public function get_table_single_row($table_name = "", $where_conditions = [] )
        {
            $current_page = 1;
            $limit = 1;
        ...
        {   
            ...
            # SELECT * FROM sigpromj_database.cvmj_view_visitant_and_visit 
            ...
            )
            {
                ...
                foreach ($filter as $field => $value ) {
                    ...
                    $keyword[$counter] = is_string($keyword[$counter]) ? "'$keyword[$counter]'" : (int)$keyword[$counter];
        ...
        public function insert($table_name = "", $array = [] )
        {
            ...
            if ($this->conn && ( is_array($array) && count($array) > 0 ) )
            {
                ...
                // verifica que la nueva insercion exista
                if ( $this->new_row_exists($table_name, $array) )
                {
                    return true;
                }
        ...
        {            
            if ($table_name !== "" && count($array) > 0 && ( isset($array['id']) || isset($array['id_user']) || isset($array['id_visitant']) ) )
            {                  
                if ( isset($array['id']) )
                {
                ...
                else if ( isset($array['id_user']) )
                {
                    $query = "SELECT * FROM $table_name WHERE id_user = " . $array['id_user'] . " ORDER BY id DESC LIMIT 1";
                }
        ...
        public function update($table_name = "", $array = [] )
        {
            ...
            if (
                $this->conn && 
                ( count($array) > 0 ) && 
                ( isset($array['id']) || isset($array['id_user']) || isset($array['id_visitant']) ) 
                )
            {
                
                $id = isset( $array['id'] ) ? $array['id'] : '';
                $id_user = isset($array['id_user']) ? $array['id_user'] : '';
                ...
                foreach($array as $key => $value)
                {
                    if ( $i_counter+1 !== count($array) && ( $key !== "id" || $key !== "id_user" || $key !== "id_visitant" ) )
                    {                        
                        $query .= "$key = ";
                        $query .= is_string($value) ? "'$value', " : "$value, ";
                    }
                    else if ( $key == "id_visitant" && isset($array['id_visitant']) )
                    {
                        $query .= "$key = ";
                        $query .= is_string($value) ? "'$value' " : "$value ";
                        $query .= "WHERE id_visitant = " . $id_visitant;
                    }
                    else if ( $key !== "id_user" && isset($array['id_user']) )
                    {
                        $query .= "$key = ";
                        $query .= is_string($value) ? "'$value' " : "$value ";                        
                        $query .= "WHERE id_user = " . $id_user;
                    }
                    else if ( $key !== "id" && isset($array['id']) )
                    {
                        $query .= "$key = ";
                        $query .= is_string($value) ? "'$value' " : "$value ";
                        $query .= "WHERE id = " . $id;
        ...
        public function create_schema_by_files_scripts( $array_with_sql_scripts_files )
        {
            ...
            if ( $this->conn && is_array( $array_with_sql_scripts_files) && count($array_with_sql_scripts_files) > 0 )
            {
                try {
                    ...
                    // se invoca un a coenxion mysqli para poder crear los trigger y store procedure;
